...
class InputFeatures(object):
    ...
    def __init__(self, input_sents, input_mask, segment_ids, speaker_ids, cls_positions, label_id, label_weights):
...
def convert_examples_to_features(examples, max_seq_length, max_utr_num, tokenizer):
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        label_id = []
        for label_utr in example.label:
            assert len(label_utr) <= max_utr_num
            while len(label_utr) < max_utr_num:
                label_utr.append(0)
            assert len(label_utr) == max_utr_num
            label_id.append(label_utr)
        ...
        label_id_flat = []
        label_weights = []
        for label_utr in label_id:
            label_id_flat.extend(label_utr)
            if sum(label_utr) > 0:
                label_weights.append(1.0)
            else:
                label_weights.append(0.0)
        assert len(label_id_flat) == max_utr_num * max_utr_num
        assert len(label_weights) == max_utr_num
...
def write_instance_to_example_files(instances, output_files):
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        features["label_weights"] = create_float_feature(instance.label_weights)
...
if __name__ == "__main__":
    ...
    for (filename, filetype) in zip(filenames, filetypes):
        ...
        new_filename = filename[:-5] + "_ar.tfrecord"
